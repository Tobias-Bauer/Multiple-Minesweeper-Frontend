...
export default class Home extends React.Component {
    ...
    componentDidMount() {
        this.connection = new WebSocket('ws://127.0.0.1:8000/create');
    ...
    render() {
        return (
            <div>
                ...
                <button onClick={() => this.connection.send(JSON.stringify({ n_cols: this.state.n_cols, n_rows: this.state.n_rows, n_mines: this.state.n_mines, code: this.state.code, solvable: this.state.solvable }))}>Create</button>
                <p>Join an existing game</p>
                <input value={this.state.code} onChange={(e) => this.setState({ code: e.target.value })} type="number" placeholder="Code to join" />
                <button onClick={() => this.connection.send({ code: this.state.code })}>Join</button>
                {this.state.messages}
            </div>
        )
    }
